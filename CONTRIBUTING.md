...
After merge, the release source is the full commit on `main` that first introduced the immutable plan and bumped versions. Once that exact commit has passed Node CI, create the draft from synchronized `main`:
...
The command immediately creates the tag and draft Release at the plan-introduction commit; later `main` commits belong to the next release. It refuses to run unless exactly one unresolved committed plan exists, and it refuses rather than overwrites an existing tag or draft. A maintainer may edit only narrative notes in the draft. Publishing the stable Release starts the npm publication workflow; saving or editing a draft never publishes packages. Investigate partial remote state after a post-creation failure, and after partial publication retry the same published Release workflow for the same tag instead of creating a replacement.
