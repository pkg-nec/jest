...
If you want to skip all the news and just get going, run `npm install @pkg-nec/jest@^30.0.0` and follow the migration guide: [Upgrading from Jest 29 to 30](https://jestjs.io/docs/upgrading-to-jest30).
...
- `@pkg-nec/jest-environment-jsdom` was upgraded from jsdom 21 to 26.
- The minimum compatible TypeScript version is now 5.4.
- Various `@pkg-nec/expect` aliases were removed. [`eslint-plugin-jest` has an autofixer](https://github.com/jest-community/eslint-plugin-jest/blob/HEAD/docs/rules/no-alias-methods.md) which you can run to automatically upgrade your codebase.
...
The default in Jest is `globalsCleanup: 'soft'`. To disable this feature you can set it to `off`. If you need to protect specific global objects from being cleaned up -- for example, shared utilities or caches -- you can mark them as protected using `@pkg-nec/jest-util`:
