...
In the next major, we plan to also eliminate `@pkg-nec/jest-jasmine2` and `@pkg-nec/jest-environment-jsdom` from the Jest dependency tree and require them to be installed explicitly, so that many users can profit from a smaller install size with less clutter that they don't need.
...
A lot of Jest's packages have been migrated to use ESM-style exports (although they are still shipped as CommonJS), so if you consume e.g. `@pkg-nec/pretty-format` directly, you may need to adjust your import to a `default` import.
