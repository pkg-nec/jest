...
- **Added:** `@pkg-nec/pretty-format` and `jest-editor-support` were merged into Jest.
...
- **Added:** Implemented file watching in `@pkg-nec/jest-haste-map`.
...
- **Fixed:** `@pkg-nec/babel-plugin-jest-hoist` when using `jest.mock` with three arguments.
- **Fixed:** The `JSON` global in `@pkg-nec/jest-environment-node` now comes from the vm context instead of the parent context.
...
- **Fixed:** global built in objects in `@pkg-nec/jest-environment-node` now work properly.
- **Fixed:** Create mock objects in the vm context instead of the parent context.
- **Fixed:** `.babelrc` is now part of the transform cache key in `@pkg-nec/babel-jest`.
