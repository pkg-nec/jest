...
3. `verify` checks the complete published batch against both the exact ledger SHA-512 integrities and the required npm provenance.
4. `evidence` runs after successful validation even if publish or verify fails, and attaches the workflow summary, ledger, publication journal, and available registry-integrity and provenance evidence to the GitHub Release.
...
There are no serial post-publish waits between packages. A resumed publish first inspects the exact `name@version`: it can continue only when an already-present version has the ledger's exact SHA-512 integrity, recording it as `verified-existing`; any differing integrity is terminal and requires investigation. After publication, verification queries the public npm registry fairly in batches of at most eight under one shared 480-second deadline for the whole 55-package batch. Package or attestation absence, network timeouts, rate limits, and registry server errors are retryable only while time remains before that shared deadline. Package or attestation absence at the deadline is terminal and produces the available partial evidence.

Registry-integrity evidence and provenance evidence are separate review records. `registry-evidence.json` and `registry-evidence.md` establish the exact public package identity and SHA-512 tarball integrity from the ledger. `provenance-evidence.json` and `provenance-evidence.md` establish the cryptographic bundle and normalized claims for the same artifact. Each package's provenance must identify the public `pkg-nec/jest` repository, the `release.yml` workflow, the expected release tag ref and source commit, a GitHub-hosted runner in a publicly visible build environment, SLSA provenance for the package artifact, and a valid npm publish attestation.

An invalid signature or transparency-log proof, malformed bundle, artifact or integrity mismatch, package identity mismatch, or unexpected repository, workflow, tag ref, source commit, runner, public visibility, SLSA provenance, or npm publish-attestation claim is immediately fatal rather than retryable. Normal releases must publish through this provenance workflow: a manual npm publication without the required provenance can never complete verification, even when its tarball integrity matches. The verify job always uploads whatever registry-integrity and provenance files were produced, and the evidence job conditionally retains those available partial files as durable GitHub Release assets after a verification failure.

Publication progress has two observability channels. The live Actions log emits a package line only after that package's journal entry has been durably promoted, and it keeps `published` (a fresh successful publish) distinct from `verified-existing` (an already-present package whose integrity matches the ledger). The publish job also appends a guarded aggregate to the GitHub Actions step summary when the durable journal exists. Its totals count selected and completed packages from the authoritative ledger and journal, including partial publication, rather than assuming a 55-package release.
